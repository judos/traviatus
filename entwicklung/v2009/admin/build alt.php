...
//Daten des Spielers
$sql="SELECT * FROM `tr_user` WHERE `id`='$userid';";
...
//Daten des Dorfes
$sql="SELECT * FROM `tr_dorfer` WHERE `x`='$dorfx' AND `y`='$dorfy';";
$result=mysql_query($sql);
$dorf_data=mysql_fetch_array($result);

//Daten des Landes
$sql="SELECT `typ` FROM `tr_lander` WHERE `x`='$dorfx' AND `y`='$dorfy';";
$result=mysql_query($sql);
...
$sql="SELECT `geb` FROM `tr_land_typen` WHERE `typ`='".$land_data['typ']."';";
$result=mysql_query($sql);
...
$sql="SELECT * FROM `tr_bau` WHERE `x`='$dorfx' AND `y`='$dorfy';";
$result=mysql_query($sql);
$build=1;
if (mysql_num_rows($result)==2)
{
    ...
    $build=0;
    if ($data1['gid']==$gid) $build=-1;
    if ($data2['gid']==$gid) $build=-1;
}
...
$forsch=0;
$sql="SELECT * FROM `tr_others` WHERE `x`='$dorfx' AND `y`='$dorfy' AND `typ`='5';";
$result=mysql_query($sql);
...
$recrut=0;
$sql="SELECT * FROM `tr_others` WHERE `x`='$dorfx' AND `y`='$dorfy' AND `typ`='1' ORDER BY `zeit` ASC;";
$result=mysql_query($sql);
...
$recrut2=0;
$sql="SELECT * FROM `tr_others` WHERE `x`='$dorfx' AND `y`='$dorfy' AND `typ`='2' ORDER BY `zeit` ASC;";
$result=mysql_query($sql);
if (mysql_num_rows($result)>0)
{
	$recrut2=1;
...
$recrut3=0;
$sql="SELECT * FROM `tr_others` WHERE `x`='$dorfx' AND `y`='$dorfy' AND `typ`='3' ORDER BY `zeit` ASC;";
$result=mysql_query($sql);
if (mysql_num_rows($result)>0)
{
	$recrut3=1;
...
$recrut4=0;
$sql="SELECT * FROM `tr_others` WHERE `x`='$dorfx' AND `y`='$dorfy' AND `typ`='4' ORDER BY `zeit` ASC;";
$result=mysql_query($sql);
if (mysql_num_rows($result)>0)
{
	$recrut4=1;
...
$festen=0;
$sql="SELECT * FROM `tr_others` WHERE `x`='$dorfx' AND `y`='$dorfy' AND `typ`='8';";
$result=mysql_query($sql);
if (mysql_num_rows($result)>0)
{
	$festen=1;
	...
	if ($festen==0)
	{
        ...
        if ($lager[0]>=$cost[$x][0] AND $lager[1]>=$cost[$x][1] AND $lager[2]>=$cost[$x][2] AND $lager[3]>=$cost[$x][3])
        {
            if ($x==1 OR ($x==2 AND $gebeude[24]['highest']>9))
            {
                ...
                $sql="UPDATE `tr_dorfer` SET `lager`='$lager_string' WHERE `x`='$dorfx' AND `y`='$dorfy';";
                $result=mysql_query($sql);

                //Fest speichern
                $sql="INSERT INTO `tr_others` (`x`,`y`,`typ`,`id`,`zeit`)
                    VALUES ('$dorfx','$dorfy','8','$x','".date('Y-m-d H:i:s',time()+$zeit[$x])."');";
                $result=mysql_query($sql);
...
{
	$ok=0;
	$sql="SELECT * FROM `tr_angebote` WHERE `ursprung`='".$_GET['u']."' AND `angebot`='".$_GET['a']."' AND
	...
	if (mysql_num_rows($result)>0)
	{
		...
		if ($lager[$nachfrage[0]-1]>=$nachfrage[1])
		{
            ...
            $sql="SELECT sum(`handler`) FROM `tr_handler` WHERE `ursprung`='$dorfx:$dorfy';";
            $result=mysql_query($sql);
            $data=mysql_fetch_array($result);
            $handler_gebraucht=$data['sum(`handler`)'];
            $sql="SELECT sum(`handler`) FROM `tr_angebote` WHERE `ursprung`='$dorfx:$dorfy';";
            $result=mysql_query($sql);
            $data=mysql_fetch_array($result);
            ...
            if (ceil($nachfrage[1]/$tragen)<=$anz_ver_handler)
            {
                ...
                $sql="SELECT `name`,`volk` FROM `tr_user` WHERE `name`='$username' OR `name`='".$data_ang['user']."';";
                $result=mysql_query($sql);
                ...
                //Angebot löschen
                $sql="DELETE FROM `tr_angebote` WHERE `ursprung`='".$_GET['u']."' AND `angebot`='".$_GET['a']."' AND
                ...
                $sql="INSERT INTO `tr_handler` (`user`,`ursprung`,`von`,`nach`,`start`,`ziel`,`handler`,`ress`)
...
if ($_GET['do']=='deloffer')
{
	...
	$sql="DELETE FROM `tr_angebote` WHERE `ursprung`='$dorfx:$dorfy' AND `angebot`='".$_GET['a']."' AND
			...
			if ($typ1!=$typ2)
			{
				if ($r1<=$lager[$typ1-1])
				{
					if ($r1/$tragen<=$anz_ver_handler)
					{
						...
						$max=10;
						if ($d1==1) $max=$d2;
						$sql="INSERT INTO `tr_angebote` (`user`,`ursprung`,`angebot`,`nachfrage`,`handler`,`maxzeit`,
...
if ($_GET['do']=='sendgoods')
{
	...
	$sql="UPDATE `tr_dorfer` SET `lager`='".$lager[0].":".$lager[1].":".$lager[2].":".
...
if ($_GET['do']=='research')
{
		...
		$lager[2]>=$troops[$rid]['forsch_eisen'] AND $lager[3]>=$troops[$rid]['forsch_getreide'] AND $forsch==0)
	{
		...
		$sql="UPDATE `tr_dorfer` SET `lager`='$lagerstring' WHERE `x`='$dorfx' AND `y`='$dorfy';";
		$result=mysql_query($sql);
		$zeit=date('Y-m-d H:i:s',time()+$troops[$rid]['forsch_zeit']);
		$sql="INSERT INTO `tr_others` (`x`,`y`,`typ`,`id`,`zeit`) VALUES ('$dorfx','$dorfy','5','$rid','".
			$zeit."');";
		$result=mysql_query($sql);
	...
	if (isset($tid))
	{
		...
		if ($anzahl>0)
		{
                ...
                AND $lager[2]>=$troops[$tid]['kosten_eisen']*$anzahl AND $lager[3]>=$troops[$tid]['kosten_getreide'])
            {
                ...
                $sql="INSERT INTO `tr_others` (`x`,`y`,`typ`,`id`,`zeit`,`anzahl`,`dauer`)
	...
	if ($stufe>0 OR ($stufe==0 AND $id<=4))
	{
        ...
        elseif ($id==16)	//Versammlungsplatz
        {
			...
			function print_troops($dorf_name,$x,$y,$titel,$volk,$army,$unterhalt)
			{
				...
				echo'<table class="tbg" cellpadding="2" cellspacing="1"><tbody>
					...
					<td>&nbsp;</td>';
				for ($i=1;$i<=10;$i++)
					echo'<td><img src="img/un/u/'.($i+$volk*10).'.gif" title="'.$troops[$i+$volk*10]['name'].'"></td>';
				echo'<td><img src="img/un/u/hero.gif" title="Held"></td>
					</tr>
					<tr><td>Einheiten</td>';
				for ($i=1;$i<=10;$i++)
				{
					if ($army[$i+$volk*10]>0) echo'<td>'.$army[$i+$volk*10].'</td>';
					else	echo'<td class="c">0</td>';
				}
				if (!isset($army['hero'])) $army['hero']=0;
				echo'<td>'.$army['hero'].'</td>
					</tr>
					<tr class="cbg1"><td>Unterhalt</td>
					<td class="s7" colspan="11">'.$unterhalt.'<img class="res" src="img/un/r/4.gif">pro Stunde</td>
					</tr>
					</tbody></table><p></p>';
			}
			...
			{
                echo'<br>';
                print_troops($dorf_data['name'],$dorfx,$dorfy,'Eigene Truppen',$spieler_volk-1,
                    $troops_village['own'],$troops_village['own']['versorgung']);

                $sql="SELECT `name`,`id`,`volk` FROM `tr_user`;";
                $result=mysql_query($sql);
                for ($i=1;$i<=mysql_num_rows($result);$i++)
                {
                    $data=mysql_fetch_array($result);
                    ...
                    {
                        print_troops($dorf_data['name'],$dorfx,$dorfy,'Truppen von '.$data['name'],$data['volk']-1,
			...
			{
				echo'<table class="p1" style="width: 100%;" cellpadding="0" cellspacing="1"><tbody>
					<tr><td>
					<table class="f10" width="100%">
					<form method="post" name="snd" action="a2b.php?do=send">
					<tbody>
					';
                ...
                <? echo'X: <input class="fm" name="x" value="'.$x.'" size="2" maxlength="4" type="text">
                Y: <input class="fm" name="y" value="'.$y.'" size="2" maxlength="4" type="text"> '; ?>
                </div></td></tr></tbody></table>

                <p><input value="ok" name="s1" src="img/de/b/ok1.gif" onmousedown="btm1('s1','','img/de/b/ok2.gif',1)" onmouseover="btm1('s1','','img/de/b/ok3.gif',1)" onmouseup="btm0()" onmouseout="btm0()" border="0" height="20" type="image" width="50"></p></form>
                <?
        ...
        elseif ($id==17)	//Marktplatz
        {
            ...
            if ($_GET['s']==1)
            {
                ...
                if ($need_handler>0)
                {
                    if ($need_handler<=$anz_ver_handler)
                    {
                        ...
                        {
                            $sql="SELECT * FROM `tr_dorfer` WHERE `name`='$dname';";
                            $result=mysql_query($sql);
                            if (mysql_num_rows($result)==1)
                            {
                                $data=mysql_fetch_array($result);
                        ...
                        if ($dname=='' AND $x!='' AND $y!='')
                        {
                            $sql="SELECT * FROM `tr_dorfer` WHERE `x`='$x' AND `y`='$y';";
                            $result=mysql_query($sql);
                            if (mysql_num_rows($result)==1)
                            {
                                $data=mysql_fetch_array($result);
                        ...
                        if ($error=='')
                        {
                            $sql="SELECT `name` FROM `tr_user` WHERE `id`='$user';";
                            $result=mysql_query($sql);
                            $data=mysql_fetch_array($result);
            ...
            $nach=split(':',$data['nach']);
                    ...
                    {
                        $sql2="SELECT `name`,`user` FROM `tr_dorfer` WHERE `x`='".$ziel[0]."' AND `y`='".$ziel[1]."';";
                ...
                $sql="SELECT * FROM `tr_handler` WHERE `nach`='$dorfx:$dorfy' AND `ursprung`!='$dorfx:$dorfy'
                    ORDER BY `ziel` ASC;";
                $result=mysql_query($sql);
                $j=0;
                if (mysql_num_rows($result)>0)
                ...
                $sql="SELECT * FROM `tr_handler` WHERE `ursprung`='$dorfx:$dorfy' ORDER BY `ziel` ASC;";
                $result=mysql_query($sql);
			...
			if ($_GET['s']==2)
			{
				...
				$sql="SELECT `name`,`volk` FROM `tr_user`;";
				$result=mysql_query($sql);
				$speed_volk=array(1=>16,2=>12,3=>24);
				for ($i=1;$i<=mysql_num_rows($result);$i++)
				{
				...
				$sql="SELECT * FROM `tr_angebote` WHERE `user`!='$username';";
				$result=mysql_query($sql);
				$anz=0;
			...
			if ($_GET['s']==3)
			{
				...
				$sql="SELECT * FROM `tr_angebote` WHERE `ursprung`='$dorfx:$dorfy';";
				$result=mysql_query($sql);
				if (mysql_num_rows($result)>0)
				{
			...
			for ($i=1;$i<=10;$i++)
			{
				...
				if ($troops[$tid]['ok2']==1 AND $troops[$tid]['typ']==1)
				{
					...
					echo'<tr><td>
						...
						onclick="document.snd.re['.($i-1).'].click();"></td>
						<td><div class="f75"><a href="#" onclick="document.snd.t'.$tid.'.value='.$buildmax.
						'; document.snd.re['.($i-1).'].click();">('.$buildmax.')</a></div></td></tr>';
				}
			}
			echo'</tbody></table><p>
        ...
        elseif ($id==24)				//Ratshaus
        {
        	...
        	echo'<table class="tbg" cellpadding="2" cellspacing="1"><tbody><tr class="cbg1"><td>Rathaus</td><td>Aktion</td></tr>
        		<tr><td><table class="f10" cellpadding="0" cellspacing="2" width="100%"><tbody><tr>
        		<td class="s7"><div><a href="#">kleines Fest</a> <span class="f8">(500 Kulturpunkte)</span></div></td>
        		</tr><tr><td class="s7" nowrap="nowrap"><img src="img/un/a/x.gif" height="15" width="1">
        		...
        		'<img src="img/un/a/clock.gif" height="12" width="18"> '.zeit_dauer($zeit_kf).'</td></tr></tbody></table></td><td width="28%">';
        	if ($festen==0)
        	{
        	...
        	if ($stufe>9)
        	{
        		echo'<tr><td><table class="f10" cellpadding="0" cellspacing="2" width="100%"><tbody><tr><td class="s7"><div>
        			<a href="#">großes Fest</a> <span class="f8">(2000 Kulturpunkte)</span></div></td></tr><tr><td class="s7" nowrap="nowrap">
        			<img src="img/un/a/x.gif" height="15" width="1"><img class="res" src="img/un/r/1.gif">29700|'.
        	...
        	if ($festen==1)
        	{
        		...
        		echo'<p></p><table class="tbg" cellpadding="2" cellspacing="1"><tbody><tr class="cbg1"><td>Fest</td><td>Dauer</td><td>Fertig</td>
        			</tr><tr><td class="s7" width="44%">'.$fest_gr[$festen_data['id']].' Fest</td><td width="25%"><span id="timer1">'.
        ...
        {
        	$sql="SELECT `value` FROM `tr_diverses` WHERE `id`='neue_dorfer';";
            $result=mysql_query($sql);
            $data=mysql_fetch_array($result);
            $neue_dorfer=split(':',$data['value']);

            $sql="SELECT `einwohner`,`x`,`y`,`name` FROM `tr_dorfer` WHERE `user`='$userid';";
            $result=mysql_query($sql);
        	...
        	if ($_GET['s']==2)
        	{
        		echo'<p>Um dein Reich zu vergrößern benötigst du Kulturpunkte. Diese nehmen mit der Zeit zu. '.
        			...
        			Um ein weiteres Dorf zu gründen oder zu erobern, würdest du <b>'.($neue_dorfer[$anz_dorfer]*1000).'</b> Punkte benötigen.</p>';
        	}
        	if ($_GET['s']==3)
        	{
        ...
        elseif ($id==34)
        {
        	echo'<table cellpadding="0" cellspacing="4" width="100%"><tbody><tr><td width="250">Aktuelle Stabilität</td>
        		<td><b>'.(100+$stufe*10).'</b> Prozent</td></tr></tbody></table>';
        }
        else
    ...
    if ($stufe<$gebeude[$id]['stufen'])
    {
        ...
        if ($build==1)
        {
            ...
            else    //Zuwenig Rohstoffe
            {
                ...
                {
                    $time[0]=($kosten[0]-$lager[0])/$produktion[0]*3600;
                    $time[1]=($kosten[1]-$lager[1])/$produktion[1]*3600;
                    $time[2]=($kosten[2]-$lager[2])/$produktion[2]*3600;
                    $time[3]=($kosten[3]-$lager[3])/$produktion[3]*3600;
                    $timed=max($time);
                    $word='';
...
<!-- Dörfer -->
<? dorfer($userid,$dorfx,$dorfy); ?>


</div>

<!-- Lager -->
<? lager($dorf_data,$produktion,$lager,$lager_grosse,$troops_village['versorgung']); ?>
